...
def simulacao_visual():
    ...
    resultado_texto.insert(tk.END, f"Número de movimentos(1): {num_quicadas}\n\n")
...
def simulacao_realista():
    ...
    scene.background = vector(0.5, 0.7, 1)  # Céu azul claro
    distant_light(direction=vector(0, -1, -1), color=color.white)
    
    bola = sphere(pos=vector(-40, altura_inicial, 0), radius=3, texture="bola.png", make_trail=True, trail_color=color.black)
    solo = box(pos=vector(bola.pos.x, altura_inicial, 0), size=vector(100, 6, 100), texture=textures.wood, make_trail=True)
    ...
    print(f"\nAltura inicial(2): {altura_inicial} metros")
    print(f"Razão(2): {razao}")
    print(f"Número de movimentos(2): {num_quicadas}")
    print(f"\nSoma geométrica das alturas(2): {soma_alturas:.2f} metros")
    print(f"Soma geométrica das distâncias(2): {soma_distancias:.2f} metros")

    # Atualizando a área de resultados no Tkinter
    resultado_texto.config(state=tk.NORMAL)  # Habilita edição no Text
    resultado_texto.delete(1.0, tk.END)  # Limpa os resultados anteriores
    resultado_texto.insert(tk.END, f"Altura inicial(2): {altura_inicial} metros\n")
    resultado_texto.insert(tk.END, f"Razão(2): {razao}\n")
    resultado_texto.insert(tk.END, f"Número de movimentos(2): {num_quicadas}\n\n")
    resultado_texto.insert(tk.END, f"Soma geométrica das alturas(2): {soma_alturas:.2f} metros\n")
    resultado_texto.insert(tk.END, f"Soma geométrica das distâncias(2): {soma_distancias:.2f} metros\n")
    resultado_texto.config(state=tk.DISABLED)  # Bloqueia edição do Text
...
tk.Button(janela, text="Iniciar Simulação (Realista)", command=simulacao_realista).pack()
