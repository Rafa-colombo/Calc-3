...
tk.Button(frame_botoes, text="Iniciar Simulação (Simples)", command=lambda: simulacao_visual(var_planeta,var_bolinha,resultado_texto,razao_entry,altura_entry)).pack(side=tk.LEFT, padx=10)
tk.Button(frame_botoes, text="Simulação (g)", command=lambda: printa_g(var_planeta, var_bolinha, resultado_texto)).pack(side=tk.LEFT, padx=10)
